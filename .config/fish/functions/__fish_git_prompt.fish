...
function __fish_git_prompt --description "Prompt function for Git"
    ...
    if test -z "$__fish_git_prompt_branch_max_length"
	set __fish_git_prompt_branch_max_length 20
    end
    set -l b_ori_length (string length $b)
    set b (string sub -l $__fish_git_prompt_branch_max_length $b)
    if test $b_ori_length -gt $__fish_git_prompt_branch_max_length
	set b "$b…"
    end
    if test -n "$b"
